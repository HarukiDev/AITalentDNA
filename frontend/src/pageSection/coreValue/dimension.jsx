...
export default function Dimension({
  ...
  const filteredDimensions = dimensions.filter((dimension) => {
    return (
      ...
      (dimension.indicators &&
        (Array.isArray(dimension.indicators)
          ? dimension.indicators.some((ind) =>
              ind.toLowerCase().includes(searchQuery.toLowerCase())
            )
          : dimension.indicators.toLowerCase().includes(searchQuery.toLowerCase())))
    );
  });
  ...
  const currentItems = filteredDimensions.slice(indexOfFirstItem, indexOfLastItem);
  ...
  return (
    <div className="min-h-screen p-0 bg-transparent">
      ...
      <div className="flex flex-col items-start justify-between mb-5 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 sm:text-3xl">Dimension</h1>
          <p className="text-sm text-gray-600 sm:text-base">
            Define each core value clearly to align with your organization's culture.
          </p>
        </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="w-full text-left border-collapse">
            <thead style={{ backgroundColor: "#0C3F8B" }} className="text-white">
              <tr>
                <th className="p-3">Name Dimension</th>
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((dimension) => (
                  <tr key={dimension.id} className="hover:bg-gray-100">
                    <td className="p-3">{dimension.name}</td>
                    <td className="p-3">{dimension.description}</td>
                    <td className="p-3">
                      {Array.isArray(dimension.indicators) && dimension.indicators.length > 0
                        ? dimension.indicators.join(", ")
                        : "No Indicator"}
                    </td>
                    <td className="flex justify-center p-3 space-x-4 text-center">
                        <button
                          className="text-blue-500 hover:text-blue-700 focus:outline-none"
                          onClick={() => openEditDimensionModal(dimension)}
                          style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                        ...
                        <button
                          ...
                          onClick={() => openDeleteModal(dimension)}
                          style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                ...
                <tr>
                  <td colSpan="4" className="p-5 italic text-center text-gray-500">
